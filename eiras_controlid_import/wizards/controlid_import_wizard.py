...
# Column indices in the Control ID CSV (semicolon-separated, 0-based)
...
class ControlidImportWizard(models.TransientModel):
    ...
    skip_existing = fields.Boolean(
        ...
        default=True,
        help="Skip days that already have an attendance record for the employee.",
    )
    ...
    def action_analyze(self):
        ...
        for row in rows:
            ...
            # Skip rest days with no clock-in records
    ...
    def action_import(self):
        ...
        for line in lines:
            for entry, exit_ in (
                (line.entry1, line.exit1),
                (line.entry2, line.exit2),
            ):
                if not entry or not exit_:
                    continue
                check_in = self._to_utc(line.date, entry)
                check_out = self._to_utc(line.date, exit_)
                if self.skip_existing and Attendance.search(
                    [
        ...
        return {
            "name": _("Imported Attendance Records"),
            "type": "ir.actions.act_window",
